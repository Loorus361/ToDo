...
function AddMilestoneForm({ projectId, onDone }: { projectId: number; onDone: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await db.milestones.add({ title: title.trim(), notes: notes.trim() || undefined, done: false, projectId, order: Date.now() });
    setTitle('');
    setNotes('');
    onDone();
  }
...
function MilestoneRow({ milestone }: { milestone: Milestone }) {
  ...
  async function toggleDone(e: React.ChangeEvent<HTMLInputElement>) {
    e.stopPropagation();
    await db.milestones.update(milestone.id!, { done: !milestone.done });
  }
      ...
      {expanded && (
        <div className="pb-3 pl-10 flex flex-col gap-2">
          <input
            ...
            onBlur={(e) => e.target.value.trim() && db.milestones.update(milestone.id!, { title: e.target.value.trim() })}
            onClick={(e) => e.stopPropagation()}
          ...
          <textarea
            ...
            onBlur={(e) => db.milestones.update(milestone.id!, { notes: e.target.value.trim() || undefined })}
            onClick={(e) => e.stopPropagation()}
            rows={2}
...
export default function ProjectDetail() {
  ...
  const todos = useLiveQuery(
    () => selectedProjectId != null ? db.todos.where('projectId').equals(selectedProjectId).toArray() : [],
    [selectedProjectId]
  );
  const communications = useLiveQuery(
    () => selectedProjectId != null
      ? db.communications.where('projectId').equals(selectedProjectId).reverse().sortBy('timestamp')
      : [],
    [selectedProjectId]
  );
  const persons = useLiveQuery(() => db.persons.toArray(), []);
  const milestones = useLiveQuery(
    () => selectedProjectId != null ? db.milestones.where('projectId').equals(selectedProjectId).toArray() : [],
    [selectedProjectId]
  );
  ...
  function handleMilestoneDragEnd(event: DragEndEvent) {
    ...
    reordered.forEach((m, i) => db.milestones.update(m.id!, { order: i * 1000 }));
  }
  ...
  return (
    <div className="max-w-3xl">
      ...
      <section className="mb-8">
        ...
        {showAddMilestone && (
          <AddMilestoneForm projectId={selectedProjectId!} onDone={() => setShowAddMilestone(false)} />
        )}
      ...
      <section className="mb-8">
        ...
        {showAddTodo && (
          <AddTodoInlineForm projectId={selectedProjectId!} onDone={() => setShowAddTodo(false)} />
        )}
            ...
            <tbody>
              {todos?.map((todo) => (
                ...
                >
                  ...
                  <td className="py-2">
                    <button
                      onClick={(e) => { e.stopPropagation(); if (confirm('Todo löschen?')) db.todos.delete(todo.id!); }}
